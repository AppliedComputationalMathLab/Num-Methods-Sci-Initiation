...
\begin{multicols}{2} % This is how many columns your poster will be broken into, a portrait poster is generally split into 2 columns
  ...
  cujo valor exato é constante e igual a $\ell$. A implementação foi realizada em
  Python, utilizando aritmética IEEE 754 binary64 e recursos gráficos para
  visualizar o comportamento numérico da função.

  \begin{figure}[H]
    \centering
    \includegraphics[width=.8\linewidth]{figs/NumError1_mplt}
    \vspace{-.5cm}
  ...
  Ao avaliar $f$ usando ponto flutuante, obtemos o gráfico da
  Figura~\ref{fig:erros1}, em que o valor correto é exibido apenas até $x$
  próximo de $4,9$. A partir de aproximadamente $x = 5,1$, a função assume o
  valor zero. Isso ocorre, pois o valor $x^{10}$ é muito maior que $1$, o que
  resulta na perda de significância, produzindo $x^{10} + 1 = x^{10}$.

  Entre $4,9$ e $5,1$ observamos a ocorrência de uma oscilação caótica
  relacionada com o valor da $\ulp(x^{10})$, que é 2. Para realizar a soma
  $x^{10} + 1$, o SPF precisa representar os dois números na mesma potência.
  Porém, o valor do expoente necessário para representar $x^{10}$ está associado
  à uma ULP igual a 2, em que o número 1 não pode ser representado e precisa ser
  arredondado. Esse arredondamento faz com que $x^{10} + 1$ seja igual a $x^{10}$
  ou $x^{10} + 2$, produzindo a oscilação observada.
